...
function ListComponent({ coin }) {
  ...
  return (
        <div className='table-row' onClick={() => {navigate(`/coin/${coin?.id}`);}}>
          <Tooltip placement='bottom-start'>
            <div className='td-img td'>
              <img className='coin-img img' src={coin?.image} alt="" />
            </div >
          </Tooltip>
          <Tooltip title="Price info" placement='bottom-start'><div className='td'>
          ...
          <Tooltip title="Current Price" placement='bottom'>
            {coin?.price_change_percentage_24h > 0 ?
              <div className='curr_price td-curr-price text-center td'>${coin?.current_price?.toLocaleString()}</div> :
              <div className='curr_price td-curr-price red_curr text-center td'>${coin?.current_price?.toLocaleString()}</div>
            }
          </Tooltip>
